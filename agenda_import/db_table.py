...
import imp
import sqlite3
from data import speaker,subSession,Session
#
...
class db_table:
    ...
    session_obj = Session.Session(None,None,None,None,None,None)
    subSession_obj = subSession.subSession(None,None)
    speaker_obj = speaker.speaker(None,None)

    session_sch = {
        session_obj.id:"integer PRIMARY KEY",
        session_obj.date:"date",
        session_obj.start:"datetime",
        session_obj.end:"datetime",
        session_obj.session_title:"text",
        session_obj.location:"text",
        session_obj.description:"text"
    }
    subSession_sch = {
        subSession_obj.session_id:"integer",
        subSession_obj.subId:"integer"
    }
    speaker_sch = {
        speaker_obj.name:"text",
        speaker_obj.id:"integer"
    }
    #
    ...
    def __init__(self, name):
        ...
        self.name    = name
        if name == "Sessions":
            self.schema = self.session_sch
        elif name == "subSessions":
            self.schema = self.subSession_sch
        elif name == "speakers":
            self.schema = self.speaker_sch
        else:
            raise RuntimeError("invalid table name")
